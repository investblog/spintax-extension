/** 301.st brand logo (filled SVG, copied from redirect-inspector) and the publisher links. */
import { siteUrl } from './studio';
...
export function svg301Logo(size = 14): SVGSVGElement {
  ...
  path.setAttribute('d', LOGO_301);
  path.setAttribute('fill', 'currentColor');
  svg.appendChild(path);
  return svg;
}

const src = (medium: string): string =>
  `utm_source=${import.meta.env.BROWSER}_ext&utm_medium=${medium}&utm_campaign=spintax-outreach`;
/** The brand link follows the site's locale routing too (EN apex, RU subdomain, /{lang}/ else). */
export const SPINTAX_URL = (medium: string): string => `${siteUrl('')}?${src(medium)}`;
export const PUBLISHER_URL = (medium: string): string => `https://301.st/?${src(medium)}`;
